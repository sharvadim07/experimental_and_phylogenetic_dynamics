...
cat ${CUR_BEAST_DIR}/solve/unified_params.txt > ${CUR_BEAST_DIR}/unified_params_${CUR_ITER}.txt
...
get_unified_params_mod(){
	...
	if [ -f ./unified_params*.txt ]; then
    	    awk '{print $6}' Posterior_init_parameters_variables_${d}.txt > treeLikelihood_col.txt && paste unified_params*.txt treeLikelihood_col.txt > ${WORK_DIR}/unified_params_phy/unified_params_${d}.txt
    	fi
    ...
    #for g in $(seq 1 9);
    #do
	python3 $find_good_resudual_likelihood -r ${wR} -p `ls unified_params* | paste -d, -s -` -n ${NUM_GOOD_RES_TR_L} --only_likelihood
	mkdir -p ${WORK_DIR}/unified_params_phy/traj_and_tree_good_likelihood
	rm -rf ${WORK_DIR}/unified_params_phy/traj_and_tree_good_likelihood/*
	GOOD_PARAM_SETS=good_likelihood_result.txt
	...
	do
	    ...
	    cp ${WORK_DIR}/phy_dyn/beast_${gl_step}/solve/${mcmc_step}_phy_trajectory.txt ${WORK_DIR}/unified_params_phy/traj_and_tree_good_likelihood/GL_${gl_step}_STATE_${mcmc_step}_phy_traj.txt
	    cp ${WORK_DIR}/phy_dyn/beast_${gl_step}/tree_pictures/STATE_${mcmc_step}.png ${WORK_DIR}/unified_params_phy/traj_and_tree_good_likelihood/GL_${gl_step}_STATE_${mcmc_step}_tree.png
	done
    #done 
