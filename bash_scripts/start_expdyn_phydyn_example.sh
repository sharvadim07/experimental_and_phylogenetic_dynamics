...
`pwd`/expdyn_phydyn_v3.sh \
`pwd`/example_in \
`pwd`/example_out \
`pwd`/example_in/zODE4a.txt \
`pwd`/example_in/all_haplotypes_p5_p6_MSA_f1.fasta \
`pwd`/example_in/p5_p6_fasta_info.txt \
20 \
1 \
1.1 \
`pwd`/example_in/target_traj_p6_W_D_A_E_var.txt \
3 \
expdyn \
10 \
0.8