    ...
    with open(equations_file_name, 'r') as equations_file:        
        ...
        for line in equations_file:
            if line[:2] == 'dx':
                ...
                repl_dict[eq_name_reres.group(1)] = 'x[' + str(eq_counter) + ']'
...
def read_init_values_file(init_values_file_name):
    ...
    for par_name in par_name_list:
        if par_name + '_min' not in par_init_values_dict:
            ...
            par_init_values_dict[par_name + '_min'] = par_init_values_dict[par_var_name + '_initval']/10.0
        if par_name + '_max' not in par_init_values_dict:    
            #init_values_dict[par_var_name + '_max'] = init_values_dict[par_var_name + '_initval']
            par_init_values_dict[par_name + '_max'] = par_init_values_dict[par_var_name + '_initval']*10.0
...
def edit_equation_C_code(c_code_pattern_file_name, equations_dict, init_values_dict, params_counter):    
...
def generate_param_ranges_file(param_ranges_file_name, init_values_dict):
    with open(param_ranges_file_name, 'w') as param_ranges_file:
        pass


equations_dict = read_equations_file(args.equations)
init_values_dict = read_init_values_file(args.equations)


edit_equation_C_code(os.path.dirname(os.path.realpath(__file__)) + "/equation.c", equations_dict, init_values_dict, params_counter)
generate_param_ranges_file(param_ranges_file_name, init_values_dict)
