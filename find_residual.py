...
def read_trajectory_file(traj_file):
    ...
    for i, traj_line in enumerate(traj_file):
        ...
        if i == 0:   
            ...
            time_values_dict[float(traj_line_splitted[time_col_index])] = \
                       tuple(float(col_val) for i, col_val in enumerate(traj_line_splitted) if i != time_col_index and \
                                                                                        i != sample_col_index)
    return var_name_tuple, time_values_dict
...
def find_residual(target_traj_file_name, calc_traj_file_name):
    with open(target_traj_file_name, 'r') as target_traj_file, \
        ...
        target_var_name_tuple, target_time_values_dict = read_trajectory_file(target_traj_file)
        calc_var_name_tuple, calc_time_values_dict = read_trajectory_file(calc_traj_file)
        target_calc_time_pairs_list = []     
        for target_time in target_time_values_dict:
            target_time = float(target_time)
            min_diff = -1
            min_diff_calc_time = -1
            for i, calc_time in enumerate(calc_time_values_dict):   
                calc_time = float(calc_time)
                if i == 0:
                    min_diff = abs(target_time - calc_time)    
                    min_diff_calc_time = calc_time         
                elif abs(target_time - calc_time) < min_diff:
                    min_diff = abs(target_time - calc_time)    
                    min_diff_calc_time = calc_time
            target_calc_time_pairs_list.append((target_time, min_diff_calc_time))

        if len(target_calc_time_pairs_list) != len(target_time_values_dict):
            raise ValueError('Times in target trajectory not matched times in calc trajectory!')
        
        #TODO: Calc residial for target columns
        sum_sq_diff = 0
        for target_var_name in target_var_name_tuple:
            ...
            for target_calc_time_pair in target_calc_time_pairs_list:
                sum_sq_diff += (target_time_values_dict[target_calc_time_pair[0]][col_index_target] - \
                                calc_time_values_dict[target_calc_time_pair[1]][col_index_calc])**2
